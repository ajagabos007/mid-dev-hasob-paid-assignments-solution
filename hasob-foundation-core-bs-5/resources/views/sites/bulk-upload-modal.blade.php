...
<a id="btn-mdl-bulk-upload-site-modal" class="btn btn-xs btn-danger btn-mdl-bulk-upload-site-modal">
    <i class="zmdi zmdi-upload"></i> Bulk Upload
</a>

<div class="modal fade" id="mdl-bulk-upload-site-modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="div-site-modal-error-bku" class="alert alert-danger" role="alert"></div>
                <form class="form-horizontal" id="frm-site-modal-bku" role="form" method="POST" enctype="multipart/form-data" action="">
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div id="spinner-mdl-bulk-upload-site-modal" class="">
            ...
            <div id="div-upload-mdl-site-modal-bku" class="modal-footer">
                <hr class="light-grey-hr mb-10" />
                <button type="button" class="btn btn-primary" id="btn-upload-mdl-site-modal" value="add">Upload</button>
            </div>
...
$(document).ready(function() {
    ...
    $(document).on('click', ".btn-mdl-bulk-upload-site-modal", function(e) {
        $('#div-site-modal-error-bku').hide();
        $('#mdl-bulk-upload-site-modal').modal('show');
        $('#frm-site-modal-bku').trigger("reset");

        $("#spinner-mdl-bulk-upload-site-modal").hide();
        $("#btn-upload-mdl-site-modal").attr('disabled', false);
    });

    //Save details
    $('#btn-upload-mdl-site-modal').click(function(e) {
        e.preventDefault();
        ...
        $("#spinner-mdl-bulk-upload-site-modal").show();
        $("#btn-upload-mdl-site-modal").attr('disabled', true);
        ...
        $.ajax({
            ...
            success: function(result){
                if(result.errors){
                    $('#div-site-modal-error-bku').html('');
                    $("#spinner-mdl-bulk-upload-site-modal").show();
                    
                    $.each(result.errors, function(key, value){
                        $('#div-site-modal-error-bku').append('<li class="">'+value+'</li>');
                    });
                }else{
                    $('#div-site-modal-error-bku').hide();
