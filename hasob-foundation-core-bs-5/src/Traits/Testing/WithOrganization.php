...
trait WithOrganization
{
    ...
    public function organization($organization_data = null ){
        $organization = is_array($organization_data) ? 
                        Organization::newFactory()->create($organization_data) : 
                        organization::firstOrCreate(Organization::newFactory()->make()->toArray());
        return $organization;
    }
