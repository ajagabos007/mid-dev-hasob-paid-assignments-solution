...
class CreateSbSubscriptionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('sb_subscriptions', function (Blueprint $table) {
            ...
            $table->foreignUuid('organization_id')->constrained('fc_organizations')->cascadeOnUpdate()->cascadeOnDelete();

            $table->integer('display_ordinal')->default(0);
            // $table->foreignUuid('offer_id')->references('id')->on('sb_offers');
            $table->foreignUuid('offer_id')->constrained('sb_offers')->cascadeOnUpdate()->cascadeOnDelete();

            // $table->foreignUuid('user_id')->references('id')->on('fc_users');
            $table->foreignUuid('user_id')->constrained('fc_users')->cascadeOnUpdate()->cascadeOnDelete();

            // $table->foreignUuid('broker_id')->references('id')->on('sb_brokers');
            $table->foreignUuid('broker_id')->constrained('sb_brokers')->cascadeOnUpdate()->cascadeOnDelete();
