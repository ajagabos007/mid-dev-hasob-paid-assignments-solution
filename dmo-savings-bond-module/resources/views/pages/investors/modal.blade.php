    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div id="div-investor-modal-error" class="alert alert-danger" role="alert">ehre</div>
...
$(document).ready(function() {
    ...
    $(document).on('click', ".btn-edit-mdl-investor-modal", function(e) {
        ...
        $.get( "{{ route('sb-api.investors.show','') }}/"+itemId).done(function( response ) {     
			...
			$('#txt-investor-primary-id').val(response.data.id);
            $('#broker_id').val(response.data.broker_id);
                        ...
                        error: function(data){
                            ...
                            if  (typeof data.responseJSON.errors == "object" )
                                Object.entries(data.responseJSON.errors).forEach(([key, value]) => {
                                    errors+="<P>"+`${value}` +"<p/>";
                                    $(`#${key}_input_error`).show();
                                    $(`#${key}_input_error`).html(`${value}`);

                                });
                                
                            else  errors+="<P>"+`${data.statusText}` +"<p/>";
                            
                            $('#div-investor-modal-error').html(errors);
                            $('#div-investor-modal-error').show();    
                            
                            // Output more define error for the user to see.
    ...
    $('#btn-save-mdl-investor-modal').click(function(e) {
        ...
        @endif
        // formData.append('', $('#').val());
        formData.append('broker_id', $('#broker_id').val());
