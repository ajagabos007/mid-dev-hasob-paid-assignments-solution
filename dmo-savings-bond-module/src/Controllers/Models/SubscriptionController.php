    ...
    public function index(Organization $org, SubscriptionDataTable $subscriptionDataTable)
    {
        ...
        $users = ['' => ''];
         ...
         foreach (User::all()->sortBy('first_name') as $key => $user) {
            $users = array_merge($users, [$user->id => $user->first_name." ".$user->last_name]);
        }
        $offers = ['' => ''];
         // clean sort and merge users by name
         foreach (Offer::all()->sortBy('status') as $key => $offer) {
            $offers = array_merge($offers, [$offer->id => $offer->id]);
        }

        $brokers = ['' => ''];
         // clean sort and merge users by name
         foreach (Broker::all()->sortBy('id') as $key => $broker) {
